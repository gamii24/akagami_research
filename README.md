...
- **デプロイ状態**: ✅ Active
- **最終更新**: 2026-02-05 - 📝 お知らせページをシンプル化（URLリンクのみ）

## 📢 お知らせページ（2026-02-05）

### 概要
会社からのお知らせや新着情報を管理・公開できる**シンプルなお知らせページ**です。管理画面から簡単に追加・編集・削除が可能です。

### 主な機能
- ✅ **お知らせの公開・非公開切り替え**: 下書き保存と公開を管理
- ✅ **日付指定**: お知らせの公開日を設定可能
- ✅ **URLリンク自動変換**: コンテンツ内のURLが自動的にクリック可能なリンクに変換
- ✅ **Googleドライブ画像表示**: Googleドライブの画像URLを自動的に画像として表示
- ✅ **管理画面からの簡単編集**: モーダル形式で追加・編集・削除
- ✅ **カード型デザイン**: 美しいカードレイアウトで表示
- ✅ **レスポンシブデザイン**: PC・スマートフォン完全対応

### Googleドライブ画像表示機能
お知らせの内容欄にGoogleドライブの画像URLを入力すると、自動的に画像として表示されます：
- **対応URL形式**: `https://drive.google.com/file/d/{FILE_ID}/view...`
- **共有設定**: Googleドライブで「リンクを知っている全員」に設定が必要
- **画像サイズ**: 最大幅320px（max-w-xs）で縮小表示、レスポンシブ対応
- **自動スタイル**: 角丸、シャドウ、ホバーエフェクト、遅延読み込み
- **クリック動作**: 画像をクリックするとモーダル（ポップアップ）で拡大表示
- **モーダル機能**: 
  - フルスクリーン表示で画像を大きく確認可能
  - 黒い半透明の背景
  - 閉じるボタン（×）をクリックで閉じる
  - 背景をクリックで閉じる
  - ESCキーで閉じる
...
### URLリンク機能
お知らせの内容欄にURLを入力すると、自動的にクリック可能なリンクに変換されます：
- 対応形式: `http://...` および `https://...` で始まるすべてのURL
- 外部サイトのURL、動画リンクなど（Googleドライブ画像は自動的に画像表示）
- 自動的に新しいタブで開く（`target="_blank"`）
- 青色の下線付きリンクスタイル

**使い方**:
1. Googleドライブで画像を共有設定「リンクを知っている全員」に変更
2. 共有リンクをコピー（例: `https://drive.google.com/file/d/1JygfZLmaN3ldZ7gEJLbfpbdTbkCMHSeS/view`）
3. 管理画面（https://akagami.net/admin/announcements）にログイン
4. 「新しいお知らせを追加」をクリック
5. タイトル、内容（Googleドライブの画像URLを貼り付け）、日付を入力
6. 「公開する」にチェックを入れて保存
7. 公開ページ（https://akagami.net/announcements）で画像が表示されることを確認
...
### 技術的な実装
- **サーバー側処理**: URLを検出して `<a>` タグに自動変換
- **リンクスタイル**: 青色下線付き、新しいタブで開く、URL折り返し対応
- **セキュリティ**: `rel="noopener noreferrer"` 属性でセキュアなリンク
...
- **画像読み込み最適化**
  ...
  - loading-placeholder の適切な非表示処理
  - 画像読み込み完了後のプレースホルダー削除
  - 画像キャッシュによる再読み込み防止
- **パフォーマンス向上**
  - スクロール回数削減（15枚 → 20枚表示で+33%改善）
  - データ使用量削減（コンパクトUI）
  - 画像アスペクト比の最適化（1080×1350に完全対応）
  - スムーズな画像表示（ちらつきなし）
- **ユーザー体験**
  - カード発見性の向上（一度に多くのコンテンツ表示）
  - UI密度の最適化（情報効率の向上）
  - スムーズなスクロール体験
  - 画像の歪みなし（object-contain使用）
...
### スマホ表示最適化（2026-02-05）
- ✅ **2列×10行レイアウト**: トップページで20枚のカードが一度に表示
- ✅ **カード間隔縮小**: gap-4 → gap-2（スマホのみ、PCは gap-4 維持）
- ✅ **カードサイズ最適化**: padding-bottom: 125% → 110%（縦長比率を調整）
- ✅ **影とボーダー軽量化**: shadow-lg → shadow-md、border-2 → border
- ✅ **フォントサイズ削減**: 
  - ダウンロード数: 0.7rem → 0.6rem
  - 日付表示: text-xs → 0.6rem
  - お気に入りアイコン: 0.865rem → 0.75rem
- ✅ **余白削減**: すべての要素のパディングとマージンを1pxずつ削減
- 📱 **効果**: スマホで一度に見えるカード数が15枚 → 20枚に増加（33%向上）
