    ...
    <div class="min-h-screen" style="background-color: #1a1a1a;">
      ...
      <main class="max-w-7xl mx-auto px-4 py-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-4 flex flex-wrap gap-2">
          ...
          <button onclick="showNewsManagementModal()" class="px-4 py-2 text-sm rounded-lg transition-all duration-300 shadow-lg font-semibold" style="background-color: #f59e0b; color: white;" aria-label="ニュース管理">
...
// Show news management modal
async function showNewsManagementModal() {
  try {
    // Fetch news articles
    ...
    modal.innerHTML = `
      <div class="flex items-center justify-center min-h-screen px-4 py-8">
        <div class="relative rounded-xl shadow-2xl w-full max-w-5xl" style="background-color: #2d2d2d; border: 2px solid #4b5563;">
          <!-- Header -->
          <div class="px-6 py-4 flex items-center justify-between" style="background-color: #3a3a3a; border-bottom: 2px solid #4b5563;">
            <h3 class="text-xl font-bold" style="color: #f3f4f6;">
              <i class="fas fa-newspaper mr-2" style="color: #f59e0b;"></i>ニュース管理
            </h3>
            <button onclick="closeNewsModal()" class="text-gray-400 hover:text-white transition-colors" aria-label="閉じる">
              <i class="fas fa-times text-2xl"></i>
            </button>
          </div>
          
          <!-- Add News Form -->
          <div class="p-6" style="border-bottom: 2px solid #4b5563;">
            <h4 class="text-lg font-semibold mb-4" style="color: #f3f4f6;">新規ニュース記事を追加</h4>
            <form id="add-news-form" class="space-y-4">
              ...
              <button type="submit" class="w-full py-3 rounded-lg font-semibold transition-all duration-300 shadow-lg" style="background-color: #f59e0b; color: white;">
                <i class="fas fa-plus mr-2"></i>ニュースを追加
              </button>
            </form>
          </div>
          ...
          <!-- News List -->
          <div class="p-6">
            <h4 class="text-lg font-semibold mb-4" style="color: #f3f4f6;">登録済みニュース記事（全 ${newsArticles.length} 件）</h4>
            <div id="news-list" class="space-y-4 max-h-96 overflow-y-auto">
              ...
              ` : newsArticles.map(news => `
                <div class="p-4 rounded-lg" style="background-color: #1a1a1a; border: 1px solid #4b5563;">
                  <div class="flex justify-between items-start mb-2">
                    <div class="flex-1">
                      <h5 class="font-semibold mb-1" style="color: #f3f4f6;">${escapeHtml(news.title)}</h5>
                      ...
                      <div class="flex gap-2 text-xs">
                        ...
                        <span class="px-2 py-1 rounded" style="background-color: #6b7280; color: white;">${news.language === 'en' ? '英語' : '日本語'}</span>
                        <span style="color: #9ca3af;">${new Date(news.published_at).toLocaleDateString('ja-JP')}</span>
                      </div>
                    </div>
                    <button onclick="deleteNews(${news.id}, '${escapeHtml(news.title)}')" class="ml-4 px-3 py-2 rounded text-sm transition-colors" style="background-color: #dc2626; color: white;">
                      <i class="fas fa-trash"></i>
                    </button>
                  </div>
...
async function addNewsArticle() {
  ...
  try {
    ...
    if (response.data.success) {
      ...
      setTimeout(() => showNewsManagementModal(), 500)
    }
  } catch (error) {
    console.error('Failed to add news:', error)
    showToast('ニュース記事の追加に失敗しました', 'error')
  }
}
