    ...
    if (state.viewMode === 'list') {
      ...
      >
        ...
        <div class="p-4 flex items-center gap-4">
          ...
          <div class="flex items-center gap-2">
            <button 
              onclick="showTagEditor(event, ${pdf.id})"
...
async function showTagEditor(event, pdfId) {
  event.stopPropagation()
  
  tagEditorState.pdfId = pdfId
  const pdf = state.allPdfs.find(p => p.id === pdfId)
  ...
  try {
    ...
    tagEditorState.allTags = response.data
  } catch (error) {
    console.error('Failed to load tags:', error)
    tagEditorState.allTags = []
  }
  
  // Load PDF's current tags
  try {
    const response = await axios.get(`/api/pdfs/${pdfId}`)
    tagEditorState.selectedTags = new Set(response.data.tags || [])
  } catch (error) {
    console.error('Failed to load PDF tags:', error)
    tagEditorState.selectedTags = new Set()
  }
  
  // Create and show modal
  renderTagEditorModal(pdf)
}

// Render tag editor modal
function renderTagEditorModal(pdf) {
  // Create modal if not exists
  let modal = document.getElementById('tag-editor-modal')
  if (!modal) {
    ...
    <div class="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
      <div class="p-6 border-b border-gray-200">
        <div class="flex items-center justify-between">
          <h2 class="text-2xl font-bold text-gray-800">
            <i class="fas fa-tags text-primary mr-2"></i>タグを編集
          </h2>
          <button onclick="closeTagEditor()" class="text-gray-400 hover:text-gray-600 transition-colors">
            <i class="fas fa-times text-2xl"></i>
          </button>
        </div>
        <p class="text-sm text-gray-600 mt-2">${escapeHtml(pdf.title)}</p>
      </div>
      ...
      <div class="p-6 space-y-6">
        ...
        <div>
          <label class="block text-sm font-semibold text-gray-700 mb-2">
            <i class="fas fa-plus-circle text-primary mr-1"></i>新しいタグを追加
          </label>
          <div class="flex gap-2">
            <input 
              type="text" 
              id="tag-editor-input"
              class="flex-1 px-4 py-3 border-2 border-gray-300 rounded-lg focus:outline-none focus:border-primary text-base"
              placeholder="タグ名を入力..."
              onkeypress="if(event.key==='Enter'){event.preventDefault();addTagFromEditor()}"
            />
            <button 
              onclick="addTagFromEditor()"
        ...
        <div>
          <label class="block text-sm font-semibold text-gray-700 mb-2">
            <i class="fas fa-check-circle text-green-600 mr-1"></i>選択中のタグ
          </label>
          <div id="tag-editor-selected" class="flex flex-wrap gap-2 p-4 bg-gray-50 rounded-lg min-h-[80px] border-2 border-gray-200">
        ...
        <div>
          <label class="block text-sm font-semibold text-gray-700 mb-2">
            <i class="fas fa-list text-blue-600 mr-1"></i>利用可能なタグ（クリックで選択）
          </label>
          <div class="max-h-60 overflow-y-auto p-4 bg-gray-50 rounded-lg border-2 border-gray-200">
            <div class="flex flex-wrap gap-2" id="tag-editor-available">
      ...
      <div class="p-6 border-t border-gray-200 flex justify-end gap-3">
        <button 
          ...
          class="px-6 py-3 bg-gray-200 hover:bg-gray-300 text-gray-700 rounded-lg transition-colors font-semibold"
        >
          <i class="fas fa-times mr-2"></i>キャンセル
        </button>
        <button 
          onclick="saveTagsFromEditor()" 
          class="px-6 py-3 bg-primary hover:bg-red-600 text-white rounded-lg transition-colors font-semibold"
        >
...
function toggleTagInEditor(tagName) {
  if (tagEditorState.selectedTags.has(tagName)) {
    tagEditorState.selectedTags.delete(tagName)
  } else {
    tagEditorState.selectedTags.add(tagName)
  }
  renderTagEditorTags()
}

// Add tag from editor input
...
function renderTagEditorTags() {
  // Render selected tags
  const selectedContainer = document.getElementById('tag-editor-selected')
  if (selectedContainer) {
    if (tagEditorState.selectedTags.size === 0) {
      selectedContainer.innerHTML = '<div class="text-sm text-gray-400 w-full text-center py-4">タグが選択されていません</div>'
    } else {
      selectedContainer.innerHTML = Array.from(tagEditorState.selectedTags).map(tag => `
        <span class="inline-flex items-center gap-2 px-4 py-2 bg-primary text-white rounded-full text-sm font-semibold shadow-sm">
          <i class="fas fa-tag"></i>
          ${escapeHtml(tag)}
          <button 
            onclick="removeTagFromEditor('${escapeHtml(tag)}')"
            class="hover:bg-red-700 rounded-full p-1 transition-colors"
          >
  ...
  const availableContainer = document.getElementById('tag-editor-available')
  if (availableContainer) {
    if (tagEditorState.allTags.length === 0) {
      availableContainer.innerHTML = '<div class="text-sm text-gray-400 w-full text-center py-4">タグがありません</div>'
    } else {
      availableContainer.innerHTML = tagEditorState.allTags.map(tag => {
        const isSelected = tagEditorState.selectedTags.has(tag.name)
        return `
          <button 
            onclick="toggleTagInEditor('${escapeHtml(tag.name)}')"
...
async function saveTagsFromEditor() {
  try {
    await axios.put(`/api/pdfs/${tagEditorState.pdfId}/tags`, {
      tags: Array.from(tagEditorState.selectedTags)
    }, { withCredentials: true })
    
    showToast('タグを保存しました', 'success')
    closeTagEditor()
    
    // Reload PDFs to reflect changes
    await loadPdfs()
  } catch (error) {
