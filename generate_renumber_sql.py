...
Keep unchanged: user_id 1, 9998, 9999
"""
...
print("--")
print("-- Strategy: Use a high temporary offset to avoid ID conflicts")
print("-- Step 1: Move 10000+ to temporary range (20000+)")
print("-- Step 2: Move temporary range to final range (2+)")
print()
...
for old_id in range(start_old_id, end_old_id + 1):
    ...
    print(f"-- Move user_id {old_id} -> {temp_id} (temporary)")
...
for i, old_id in enumerate(range(start_old_id, end_old_id + 1)):
    ...
    print(f"-- Move {temp_id} -> {new_id} (final: {old_id} -> {new_id})")
...
print("SELECT id, name, email FROM users WHERE id < 200 ORDER BY id;")
